...
router.post("/app/thirdParty", async function (req: Request, res: Response) {
    const accountType = req.body.accountType;
    const accountId = req.body.accountId;
    let user = await UserModel.getUserWithAccountTypeAndAccountId(accountType, accountId);
    if (user) {
        res.status(200).json(user);
    } else {
        res.status(500).json({ errors: [{ msg: "沒有此使用者！" }] })
    }
})

...
export class UserModel extends modelHelper {
   ...
   */
    ...
    /**
     * 取得用戶資料 by 平台與account id 給 third party login in app 用的
     */
    static async getUserWithAccountTypeAndAccountId(accountType: string, accountId: string): Promise<UserDocument> {
        let user = await User.findOne({ accountType, accountId });
        if (user) {
            return user
        } else {
            return null
